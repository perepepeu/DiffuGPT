...
from sample import generate, generate_ar, generate_hybrid_compat, safe_torch_load
...
def load_model(path):
    ...
    ckpt = safe_torch_load(path, map_location=cfg.device)
...
def generate_api():
    ...
    if mode == "diffusion":
        ...
        return jsonify(
            {"text": out, "tokens": tokens, "tps": round(tps, 2), "mode": "diffusion"}
        )
    ...
    # Modo 2: AR puro
        ...
        return jsonify(
            {"text": out, "tokens": tokens, "tps": round(tps, 2), "mode": "ar"}
        )

    # Modo 3: Hibrido
    elif mode == "hybrid":
        temperature = float(data.get("temperature", cfg.ar_temperature))
        threshold = float(data.get("threshold", cfg.ar_threshold_balanced))
        show = data.get("diffusion", False)

        draft, refined, dt, dtps, at, atps = generate_hybrid_compat(
            current_model,
            tok,
            cfg,
            prompt,
